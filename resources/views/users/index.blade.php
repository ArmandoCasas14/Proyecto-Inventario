            ...
            <div class="bg-emerald-600 dark:bg-emerald-700 px-6 py-6 sm:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-emerald-100 mt-1">
                        {{ __('Administra los accesos, roles y el estado operacional de las cuentas de usuario.') }}
                    </p>
                </div>

                <a href="{{ route('usuarios.create') }}" 
                   class="inline-flex items-center justify-center px-4 py-2.5 bg-emerald-800 hover:bg-emerald-900 text-white font-semibold text-xs rounded-xl transition-all shadow-sm gap-2 shrink-0">
                    ...
                    <form action="{{ route('usuarios.index') }}" method="GET">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                            ...
                            <div class="md:col-span-4">
                                ...
                                <select name="role_id" class="w-full text-xs rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 focus:border-emerald-500 focus:ring-emerald-500 py-2.5 shadow-sm">
                            ...
                            <div class="md:col-span-2 flex items-center gap-2">
                                <button type="submit" class="w-full h-[38px] bg-slate-800 hover:bg-slate-900 dark:bg-slate-200 dark:hover:bg-white text-white dark:text-slate-900 font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-sm flex items-center justify-center">
                                    {{ __('Buscar') }}
                                </button>
                                
                                @if(request()->anyFilled(['search', 'role_id']))
                                    <a href="{{ route('usuarios.index') }}" title="{{ __('Limpiar filtros') }}" 
                    ...
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700/80 text-left">
                        <thead class="bg-slate-50/80 dark:bg-slate-900/80 text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[11px] font-bold">
                            <tr>
                                ...
                                <th class="px-6 py-3.5">{{ __('Rol') }}</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50 text-xs bg-white dark:bg-slate-800">
                            @forelse($users as $user)
                                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-slate-600 dark:text-slate-300 font-mono">
                                        {{ $user->email }}
                                    </td>
                                    <td class="px-6 py-4 text-slate-600 dark:text-slate-300">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200">
                                        ...
                                        @if(auth()->user()->role->name === 'Administrador')
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <a href="{{ route('usuarios.edit', $user) }}"
                                                   title="{{ __('Editar') }}"
                                                   class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-amber-100 text-amber-700 hover:bg-amber-500 hover:text-white dark:bg-amber-900/40 dark:text-amber-300 dark:hover:bg-amber-600 dark:hover:text-white transition-all shadow-sm">
                                                ...
                                                <form action="{{ route('usuarios.toggleStatus', $user) }}" method="POST"
                                                    ...
                                                    @if($user->status)
                                                        <button type="submit" title="{{ __('Inactivar Usuario') }}"
                                                                class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-rose-100 text-rose-700 hover:bg-rose-500 hover:text-white dark:bg-rose-900/40 dark:text-rose-300 dark:hover:bg-rose-600 dark:hover:text-white transition-all shadow-sm">
                                                    ...
                                                    @else
                                                        <button type="submit" title="{{ __('Activar Usuario') }}"
                                                                class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-emerald-100 text-emerald-700 hover:bg-emerald-500 hover:text-white dark:bg-emerald-900/40 dark:text-emerald-300 dark:hover:bg-emerald-600 dark:hover:text-white transition-all shadow-sm">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-slate-500 dark:text-slate-400 italic">
