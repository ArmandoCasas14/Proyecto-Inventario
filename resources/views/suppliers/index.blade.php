            ...
            <div class="bg-emerald-600 dark:bg-emerald-700 px-6 py-6 sm:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-emerald-100 mt-1">
                        {{ __('Administra la información de contacto y NIT de tus proveedores.') }}
                    </p>
                </div>
                ...
                <div class="bg-slate-50/70 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                    <form action="{{ route('proveedores.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
                        <div class="flex-1 w-full">
                            ...
                            <div class="relative">
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar por razón social o NIT..." 
                        ...
                        <div class="w-full sm:w-48">
                            ...
                            <select name="status" class="w-full text-xs rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 focus:border-emerald-500 focus:ring-emerald-500 py-2.5 shadow-sm">
                        ...
                        <div class="flex gap-2 w-full sm:w-auto">
                            ...
                            @if(request()->anyFilled(['search', 'status']))
                                <a href="{{ route('proveedores.index') }}" class="px-5 py-2.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors text-center">
                                    {{ __('Limpiar') }}
                                </a>
                            @endif
                        </div>
                ...
                <div class="overflow-x-auto rounded-2xl border border-slate-200/80 dark:border-slate-700/80">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700/80 text-left">
                        <thead class="bg-slate-50/80 dark:bg-slate-900/80 text-slate-500 dark:text-slate-400 uppercase tracking-wider text-[11px] font-bold">
                            <tr>
                                <th class="px-6 py-3.5">{{ __('Razón Social') }}</th>
                                <th class="px-6 py-3.5">{{ __('NIT') }}</th>
                            ...
                            @forelse ($suppliers as $supplier)
                                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-slate-600 dark:text-slate-300 whitespace-nowrap">
                                        <span class="font-mono bg-slate-100 dark:bg-slate-900/60 px-2 py-1 rounded-md border border-slate-200/60 dark:border-slate-700/60">{{ $supplier->nit }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                        {{ $supplier->address ?? '—' }}
                                    </td>
                                    <td class="px-6 py-4 text-slate-600 dark:text-slate-300">
                                        <div class="font-medium text-slate-700 dark:text-slate-200">{{ $supplier->phone ?? '—' }}</div>
                                        <div class="text-[11px] text-slate-400 dark:text-slate-500">{{ $supplier->email ?? '' }}</div>
                                    </td>
