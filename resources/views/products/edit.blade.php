<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Editar Producto:') }} <span class="font-bold text-indigo-600 dark:text-indigo-400">{{ $product->name }}</span>
        </h2>
    </x-slot>
                ...
                <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="code" name="code" type="text" class="mt-1 block w-full" 
                                value="{{ old('code', $product->code) }}" required placeholder="Ej: PROD-1002" />
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" 
                                value="{{ old('name', $product->name) }}" required placeholder="Ej: Laptop Dell Inspiron" />
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" 
                            ...
                            rows="3" placeholder="Detalles o especificaciones del producto...">{{ old('description', $product->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="purchase_price" name="purchase_price" type="number" step="0.01" class="mt-1 block w-full" 
                                value="{{ old('purchase_price', $product->purchase_price) }}" required placeholder="0.00" />
                        ...
                        <div>
                            ...
                            <x-text-input id="selling_price" name="selling_price" type="number" step="0.01" class="mt-1 block w-full" 
                                value="{{ old('selling_price', $product->selling_price) }}" required placeholder="0.00" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <x-text-input id="current_stock" name="current_stock" type="number" class="mt-1 block w-full" 
                                value="{{ old('current_stock', $product->current_stock) }}" required placeholder="0" />
                        ...
                        <div>
                            ...
                            <x-text-input id="minimum_stock" name="minimum_stock" type="number" class="mt-1 block w-full" 
                                value="{{ old('minimum_stock', $product->minimum_stock) }}" required placeholder="5" />
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                <option value="1" {{ old('status', $product->status) == '1' ? 'selected' : '' }}>{{ __('Activo') }}</option>
                                <option value="0" {{ old('status', $product->status) == '0' ? 'selected' : '' }}>{{ __('Inactivo') }}</option>
                            </select>
